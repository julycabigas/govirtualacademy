	       		...
	       		<template v-else>
	       			<tr v-for="resource in resources.data" :key="resource.id">
						<td>{{ resource.id }}</td>
	       				<td>{{ resource.name }}</td>
	       				<td width="150">
	       					<router-link :to="`/admin/resources/edit/${resource.id}`" class="btn btn-custom btn-sm btn-secondary">Edit</router-link>
	       					<router-link :to="`/admin/resources/delete/${resource.id}`" class="btn btn-custom btn-sm btn-danger">Delete</router-link>
	       				</td>
	       			</tr>
	       		</template>	
	       	</tbody>
...
<script>
	export default {
		...
		methods: {
			getResults(page = 1) {
				axios.get('/api/resources?page=' + page, {
					headers: {
